...
def update_anime(anime_id: int, data: dict) -> None:
    """更新动漫信息

    Args:
        anime_id: 动漫 ID
        data: 待更新字段字典，仅允许白名单中的字段

    Raises:
        ValueError: data 中包含不允许的字段名
    """
    fields: list[str] = []
    values: list[Any] = []
    for key, value in data.items():
        if key not in _ANIME_UPDATABLE_FIELDS:
            logger.warning(f"update_anime 忽略非法字段: {key}")
            continue
        fields.append(f"{key} = ?")
        values.append(value)
    if not fields:
...
def touch_anime_sync(anime_id: int) -> None:
    """更新动漫最后同步时间"""
    with get_connection() as conn:
        conn.execute(
...
def add_alias(anime_id: int, alias: str) -> None:
    """添加别名"""
    with get_connection() as conn:
        conn.execute(
...
def set_source_rules(anime_id: int, rules: dict) -> None:
    """设置搜索规则"""
    import json
    with get_connection() as conn:
        conn.execute(
...
def add_sync_log(anime_id: int, sync_type: str, episodes_synced: int,
                 sources_found: int, status: str = "success", message: str = "") -> None:
    """添加同步日志"""
    with get_connection() as conn:
        conn.execute(
...
def cleanup_old_sync_logs(days: int = 90) -> None:
    """清理过期同步日志"""
    with get_connection() as conn:
        conn.execute(
...
def get_watch_stats() -> dict:
    ...
    with get_connection() as conn:
        ...
        episode_row = conn.execute(
            "SELECT COUNT(*) as total, SUM(watched) as watched FROM episodes"
        ).fetchone()
        total_episodes = episode_row["total"] or 0
        watched_episodes = int(episode_row["watched"] or 0)
        ...
        status_dist = [
            {
                "status": r["status"],
                "label": STATUS_LABELS.get(r["status"], r["status"] or "未知"),
                "count": r["cnt"],
            }
        ...
        progress_rows = conn.execute(
            """SELECT a.id, a.title_cn,
                      COUNT(e.id) as ep_count,
                      SUM(e.watched) as watched_count
               FROM animes a
               LEFT JOIN episodes e ON a.id = e.anime_id
               GROUP BY a.id
               HAVING ep_count > 0
               ORDER BY (CAST(SUM(e.watched) AS FLOAT) / COUNT(e.id)) DESC,
                        watched_count DESC
               LIMIT 10"""
        ).fetchall()
        top_progress = [
            {
                **dict(r),
                "pct": round(r["watched_count"] / r["ep_count"] * 100) if r["ep_count"] else 0,
            }
        ...
        pending_rows = conn.execute(
            """SELECT a.id, a.title_cn,
                      COUNT(CASE WHEN e.watched = 0 THEN 1 END) as unwatched
               FROM animes a
               LEFT JOIN episodes e ON a.id = e.anime_id
               GROUP BY a.id
               HAVING unwatched > 0
               ORDER BY unwatched DESC
               LIMIT 6"""
        ).fetchall()
        most_pending = [dict(r) for r in pending_rows]
        ...
        sync_activity = [dict(r) for r in activity_rows]
        max_sync_count = max((r["syncs"] for r in activity_rows), default=1)

    completion_rate = round(watched_episodes / total_episodes * 100, 1) if total_episodes else 0
    estimated_hours = round(watched_episodes * 24 / 60, 1)
